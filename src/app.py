...
def disease_rules(patient_dict):
    ...
    # MODIFIED: Only return Heart Attack Risk - show only if risk > 0, otherwise return "No future heart attack risk"
    heart_attack_risk = rules[2]  # Heart Attack Risk (Myocardial Infarction)
    if heart_attack_risk['risk'] > 0:
        return [heart_attack_risk]
    else:
        return []  # Return empty list, will be replaced with "No future heart attack risk" message
    ...
    *   **Clinical Guidelines:** Logic based on KDIGO 2022 and ADA 2024.
    ...
    if st.button("Analyze Patient Profile"):
        ...
        if potential_diseases:
            ...
            st.success("✅ No future heart attack risk detected for this patient.")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Run Batch Prediction"):
            ...
            temp_df = temp_df.replace(to_replace={r'\t': '', r'\?': np.nan}, regex=True)
            ...
            for idx, row in temp_df.iterrows():
                ...
                if complications:
                    ...
                    comp_str = "No future heart attack risk"
                all_complications.append(comp_str)
